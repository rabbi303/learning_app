...
class ArabicApp:
    ...
    def create_main_menu(self):
        ...
        tk.Label(self.root, text="আসসালামু আলাইকুম \n চলো খেলতে খেলতে শিখি", font=("SolaimanLipi", 28, "bold"),
    ...
    def show_learn_completion(self):
        self.clear_screen()
        tk.Label(self.root, text="Masha'Allah, \nBy the grace of Allah you did it!", font=("Arial", 32, "bold"),
                 bg="#d0f0fd", fg="green").pack(pady=40)

        tk.Button(self.root, text="🔁 Read Again", font=("Arial", 16),
...
class ArabicClickPuzzle:
    ...
    def on_choice(self, ch, btn_index):
        ...
        if ch in targets:
            ...
            if targets.issubset(selected):
                self.feedback_var.set("✅ MaSaAllah, You found the letter!")
            ...
            # 🔴 Make all non-target boxes red
            for i, choice in enumerate(puzzle["choices"]):
                if choice not in targets:
                    self.buttons[i].config(bg="red", fg="white")
    ...
    def next_puzzle(self):
        if not self.choice_made:
            self.feedback_var.set("OH! You didn't pick any letters. Please choose one.")
            self.feedback_label.config(fg="#ff6699")
            return
